...
<div class="container mt-5">
    ...
    <form:form action="/San-Pham/updateSPCT" method="post" modelAttribute="sanPhamChiTiet">
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <form:select path="idkichthuoc" class="form-select" id="idkichthuoc">
                <c:forEach var="kt" items="${kichthuoc}">
                    <option value="${kt.id}" <c:if test="${kt.id == sanPhamChiTiet.idkichthuoc.id}">selected="selected"</c:if>>${kt.ten}</option>
                </c:forEach>
            </form:select>
        </div>
        <div class="mb-3">
            <label for="idsanpham" class="form-label">ID sản phẩm</label>
            <form:input path="idsanpham" type="text" class="form-control" id="idsanpham" readonly="true" />
        ...
        <div class="mb-3">
            ...
            <form:select path="idmausac" class="form-select" id="idmausac">
                <c:forEach var="ms" items="${mausac}">
                    <option value="${ms.id}" <c:if test="${ms.id == sanPhamChiTiet.idmausac.id}">selected="selected"</c:if>>${ms.ten}</option>
                </c:forEach>
            </form:select>
        </div>
        <div class="mb-3">
            <label for="soluong" class="form-label">Số lượng</label>
            <form:input path="soluong" type="text" class="form-control" id="soluong"/>
            <div style="color: red"><form:errors path="soluong" /></div>
        </div>
        <div class="mb-3">
            <label for="dongia" class="form-label">Đơn giá</label>
            <form:input path="dongia" type="text" class="form-control" id="dongia"/>
