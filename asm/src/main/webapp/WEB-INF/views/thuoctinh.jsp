...
<div class="col-9 offset-3 p-0 position-relative">
    ...
    <div >
        <div>
            <div class="text-center mt-3"> <h5>thuộc tính</h5></div>
            <div class="ml-auto"><a href="/San-Pham/addMau" class="btn btn-success ">ADD màu</a></div>
            <table class="table">
                <thead>
                <tr>
                    <th scope="col">stt</th>
                ...
                <c:forEach var="list" items="${mauSac}" varStatus="i" >
                    <tr>
                        <td>${i.index}</td>
                        <td>${list.id}</td>
                        <td>${list.ma}</td>
                        ...
                        <td><a href="/San-Pham/formUpdatems?id=${list.id}" class="btn btn-info ">sửa</a>
                            <a href="/San-Pham/xoams?id=${list.id}" class="btn btn-info ">xóa</a></td>
                    </tr>
                </c:forEach>
                </tbody>
            </table>
        ...
        <br>
<%--      //  --------------------------  tường thành  // :>--%>
        <div class="ml-auto"><a href="/San-Pham/addTk" class="btn btn-success ">ADD kích thước</a></div>
        <table class="table">
            <thead>
            <tr>
                <th scope="col">stt</th>
            ...
            <c:forEach var="list" items="${kichthuoc}" varStatus="i" >
                <tr>
                    <td>${i.index}</td>
                    <td>${list.id}</td>
                    <td>${list.ma}</td>
                    <td>${list.ten}</td>
                    <td>${list.trangthai ? "hoạt động" : "không hoạt động"}</td>
                    <td><a href="/San-Pham/formUpdatetk?id=${list.id}" class="btn btn-info ">sửa</a>
                        <a href="/San-Pham/xoaTK?id=${list.id}" class="btn btn-info ">xóa</a></td>
                </tr>
            </c:forEach>
